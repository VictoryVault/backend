...
# Project structure
`\alembic`: alembic migrations for automated database management (autogenerated)
...
[Poetry](https://python-poetry.org/docs/) is used for package management within this project, with poetry.lock and pyproject.toml provided. Packages can be installed by calling `poetry install` from the project root.
...
This will be subject to change once authentication with the cloud DB is configured. 

Alembic should be triggered to configure the database by calling: 
```
Alembic upgrade HEAD
```
### pytest version specified to ~8.1
The recently released pytest 8.2 seems to cause problems, will wait for bug fixes before updating.
...
# Running webapp 
### Local
If appropriate env variables are set, the app can be run locally by calling `poetry run python -m run` from within the project root (`\backend`).

### Docker
From project root, run:
```bash
docker build -t vv-backend .
docker run -p 127.0.0.1:80:80 vv-backend
```

### Access running webapp
The app will be reachable at `http://localhost:80`

The autogenerated interactive api docs can be accessed at `http://localhost:80/docs` once the webapp has started, which can be useful for quick user testing of endpoints.


# Running tests
Ensure a different DB_NAME is specified as compared to the general dev DB if not using a dedicated testing PostgreSQL instance, as the test DB that is created will cause testing to fail if a db with the same name already exists. A single database connection is used for all test. A new session is generated and all tables are dropped before being recreated between each test to ensure no cross contamination between tests. 

Calling `poetry run python -m pytest` from within `\backend` will trigger all tests. 
...
Defined in `.github/workflows/pytest.yml`

Trigger on push to any branch to ensure tests are always run
```yaml
on:
  push:
    branches:
...
Run on ubuntu runner (uses less minutes than other OS):
```yaml
runs-on: ubuntu-latest
```

Start PostgreSQL service container provided by GitHub to serve as testing database within action run
```yaml
services:
  postgres:
    image: postgres
...
Run backend tests hosted on runner agent, with testing database configured to connect with the
service container created above
```yaml
